//import 'dart:math';
/// 为满足UI样式 修改自 https://pub.dev/packages/percent_indicator
import 'dart:ui';

import 'package:flutter/material.dart';
import 'dart:math' as math;

enum IWArcType { HALF, HALF_270, FULL }

enum IWCircularStrokeCap { butt, round, square }

extension IWCircularStrokeCapExtension on IWCircularStrokeCap {
  StrokeCap get strokeCap {
    switch (this) {
      case IWCircularStrokeCap.butt:
        return StrokeCap.butt;
      case IWCircularStrokeCap.round:
        return StrokeCap.round;
      case IWCircularStrokeCap.square:
...
class IWCircularPercentIndicator extends StatefulWidget {
  ...
  final IWCircularStrokeCap circularStrokeCap;
  ...
  IWCircularPercentIndicator({
    ...
    this.circularStrokeCap = IWCircularStrokeCap.butt,
  ...
  @override
  _IWCircularPercentIndicatorState createState() =>
      _IWCircularPercentIndicatorState();
}

class _IWCircularPercentIndicatorState extends State<IWCircularPercentIndicator>
  ...
  void _checkIfNeedCancelAnimation(IWCircularPercentIndicator oldWidget) {
  ...
  @override
  void didUpdateWidget(IWCircularPercentIndicator oldWidget) {
            ...
            if (widget.widgetIndicator != null && widget.animation)
              Positioned.fill(
                child: Transform.rotate(
                  angle: radians(
                      (widget.circularStrokeCap != IWCircularStrokeCap.butt &&
                          widget.reverse)
                          ? -15
                          : 0)
                      .toDouble(),
  ...
  double getCurrentPercent(double percent) {
    if (widget.arcType != null) {
      ...
      late double margin;
      if (widget.arcType == IWArcType.HALF) {
        margin = 180 * widget.percent;
      } else if(widget.arcType == IWArcType.HALF_270){
        margin = 180 * widget.percent;
      }else {
...
_ArcAngles _getStartAngleFixedMargin(IWArcType arcType) {
  double fixedStartAngle, startAngleFixedMargin;
  if (arcType == IWArcType.FULL) {
    fixedStartAngle = 220;
    startAngleFixedMargin = 172 / fixedStartAngle;
  } else if(arcType == IWArcType.HALF_270){
    fixedStartAngle = 180+45;
